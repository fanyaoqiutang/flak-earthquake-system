...
def migrate_database():
    ...
    with app.app_context():
        ...
        # 4. 插入城市数据
        cities_to_add = []
        for province_name, cities in province_city_mapping.items():
            province = Province.query.filter_by(province_name=province_name).first()
            if not province:
                print(f"⚠️  未找到省份: {province_name}")
                continue
        ...
        if cities_to_add:
            ...
            db.session.commit()
            print(f"✅ 成功导入 {len(cities_to_add)} 个城市数据")
        else:
            print(" 没有导入任何城市数据")
